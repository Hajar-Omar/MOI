...
$(document).ready(function () {
    ...
    if ($('.progress-item').length > 0) {
        ...
        var itemsInProgrss = $('.service-progress .progress-item').length;
        var percntge = 100 / itemsInProgrss;  // * 2
        $(document).on('click', '.service-progress .progress-item', function (e) {
            e.preventDefault();
            var progClasses = document.querySelector('.service-progress-contnr').classList;

            $('.presentage-val span').text((percntge * ($(this).index() + 1)).toFixed(0));
            $('.presentage-val').css('left', (percntge * ($(this).index() + 1)) - 7+ '%');

            $('.progress-item').removeClass('active');
            $('.progress-item').eq($(this).index()).addClass('active');

            //remove active-^ classes
            for (let i = 1; i < progClasses.length; i++) {
                $('.service-progress-contnr').removeClass(''+progClasses[i]);
            }
            $('.service-progress-contnr').addClass('active-'+$(this).index() + 1);

            var tagetIndex = $(this).index();
            $('.service-progress-contnr .progress-item').css('background', '#fff');
            $('.service-progress-contnr .progress-item span').removeClass('active');
            $('.service-progress-contnr .progress-item').each(function () {
                if ($(this).index() <= tagetIndex) {
    ...
    if ($('.images-gall-owl').length > 0) {
        ...
        //        $('.images-gall-owl.slick-carousel .slick-track').attr('style', 'opacity: 1; transform: none;display: flex; justify-content: center;');
        //        $('.images-gall-owl.slick-carousel').css('box-shadow','none')
        //    }
        ...
        $('.slider-for').on('afterChange', function (event, slick, direction) {
            if (direction == 'left') {
                $('.current-img').text($('.slider-for .slick-current').data('slick-index') - 1)
            }
            else {
                $('.current-img').text($('.slider-for .slick-current').data('slick-index') + 1)
            }
    ...
    $('.genericContent >div:nth-of-type(2) a[name]').each(function () {
        $(this).attr('id', $(this).attr('name')).attr('name', '')
    });

    $('.genericContent >div:nth-of-type(2) a[href^="#"]').on('click', function (event) {
        var target = $(this.getAttribute('href'));
        if (target.length) {
            event.preventDefault();
            if ($('header#header').offset().top < 65) {
